This comprehensive stability fix addresses the issues of rapid key presses, UI panel state conflicts, lack of debouncing, and multiple dispatch calls by introducing a robust keyboard hook, a utility for input stabilization, and an error boundary for UI components.

---

### 1. `src/utils/inputStabilizer.ts`

This file provides core utilities for debouncing, throttling, managing critical sections (to prevent state conflicts), and queuing actions (to prevent multiple dispatch calls from overwhelming the system).
